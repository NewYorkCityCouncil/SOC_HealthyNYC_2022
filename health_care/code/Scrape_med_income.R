# Setup / This scraping method requires RSelenium and Docker
...
##### 
#Save the scrape so don't have to do over and over again
# Change table column names to proper names
#col_names <- c(paste("Area","Number",c("Estimate","MOE","CV"),sep = "_"),
#               paste("NYC","Percent",c("Estimate","MOE"),sep = "_"))

#saveRDS(pop_tables, file = "../data/nta_info_tables.rds")
#####
...
med_income <- data.frame(NTA2020 = nta_names,
                         ...
                         num_households = lapply(pop_tables,function(x) x[1,2]) %>% unlist) %>%
  mutate_all(na_if,"")
...
rm(nta_names,URL,URL_end,pop_tables,temp_html,temp_URL,remDr,med_income,i)
